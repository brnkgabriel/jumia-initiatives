var Featurebox = function (json) {
  class Util {
    ...
    initialize() {
      ...
      this.FILTER = "filter";
      this.JUMIA_LOGO =
        "https://ng.jumia.is/cms/0-1-homepage/jumia-new-logo.png";
       ...
       * @returns {HTMLElement}
       */
      this.el = (query, parent) =>
        parent ? parent.querySelector(query) : document.querySelector(query);
      /**
       *
       * @param {string} query
       * @param {string} parent
       ...
       * @returns {string}
       */
      this.filtersUrl = (json) =>
        json.host + "/catalog/filters/?return=" + json.url;
      ...
      this.skusPerPage = () => {
        const skusPP =
          json.config["skus_per_page_" + this.NAME] ||
          json.config.skus_per_page ||
          40;
    ...
    nameMap() {
      return {
        ...
        "country_local--jumia_global":
          this.config.country_name + " & " + "abroad",
        "jumia_global--country_local":
          this.config.country_name + " & " + "abroad",
        brand: "brand",
        rating: "rating",
        price: "price",
    ...
    url(json, domain) {
      const catalog =
        json.search !== "" ? domain.catalogSearch : domain.catalog;
  ...
  class PreProcess {
    ...
    proceed() {
      ...
      hello.innerHTML =
        hTxtC === "Your"
          ? "<icon></icon><txt>Your</txt>"
          : "<icon></icon><txt>Hi,</txt>";
  ...
  class Controller {
    ...
    initialize(json) {
      ...
      this.productsFound = util.el(".-catalog") || {
        style: { display: "block" },
      };
    ...
    freelinks() {
      ...
      const freelinks = this.json.json_list.filter(
        (datum) => datum.initiative === util.FREELINK_NAME
      );
      frlinkParent.innerHTML = freelinks
        .map(this.freelinkHTML.bind(this))
        .join("");
    ...
    renderShells() {
      ...
      const dbStatus =
        this.config["display_double_banner_" + util.NAME] ||
        this.config["display_double_banner"];
      const flStatus =
        this.config["display_collection_icons_" + util.NAME] ||
        this.config["display_collection_icons"];
      const ctStatus =
        this.config["display_catalog_" + util.NAME] ||
        this.config["display_catalog"];
      ...
      const skeletons = {
        ...
        catalog: () =>
          util.isMobile ? this.mobileCatalog() : this.desktopCatalog(),
      };
    ...
    start(directUrl) {
      util.isMobile
        ? this.fetch().mobile(directUrl)
        : this.fetch().desktop(directUrl);
    }
    ...
    fetch() {
      ...
      const passesFilter = (datum) => {
        const rcs = util.remContent.filter(
          (rc) => datum.categories.indexOf(rc) > -1
        );
        return rcs.length === 0 ? true : false;
      };

      const tagData = () =>
        this.json.json_list.filter((item) => item.initiative === util.TAG);
      ...
      const isAtScrollEdge = (products) =>
        products.scrollLeft + products.clientWidth >= products.scrollWidth;
      ...
      const scrollHandler = (products) =>
        isAtScrollEdge(products) && loadMore();

      const scrollListener = () => {
        this.products.addEventListener("scroll", () =>
          scrollHandler(this.products)
        );
      };
        ...
        mobile: (directUrl) => {
          ...
          const mobileScraping = (beautified) => {
            return [
              ...
              scrape(json.url).then((data) => mobileExtract(data, json.type))
            );
          };
          ...
          const list = (json) => {
            ...
            return filters
              ? filters.find((item) => item.id === json.type).options
              : [];
          };
          ...
          const reducer = (list) => {
            return list.reduce((acc, category) => {
              ...
              const link = category.value
                .split("-")
                .filter((pieces) => isNaN(pieces))
                .join("-");
              acc[name] = { name, link };
              return acc;
            }, {});
  ...
  const Filters = function () {
    class FilterBar {
      ...
      initialize() {
        ...
        this.filtersTrigger.addEventListener("click", () =>
    ...
    class AppliedFilters extends Filter {
      ...
      updateUi() {
        const neededKeys = Object.keys(util.urlParams).filter(
          this.unNeededKeys.bind(this)
        );

        const neededObjs = neededKeys.map(this.keyValueObj.bind(this));

        const neededHTMLs = neededObjs.map(this.html.bind(this)).join("");
      ...
      html(json) {
        ...
        return `<div id="${idParam}" data-key="${json.key}" data-value="${
          json.value
        }" class="-applied-filter" data-name="${util.id(idParam)}">${
          nameMap[json.key]
        }: ${nameMap[json.value] || json.value}</div>`;
      }
    }
    ...
    class CategoryFilter extends Filter {
      ...
      html(datum) {
        ...
        return `<div data-url="${url}" id="${util.id(
          name
        )}" data-name="${name}" class="${className}">${name}</div>`;
      }
    ...
    class BrandFilter extends Filter {
      ...
      checked() {
        ...
        const brands = Array.from(allBrands).filter(
          (brand) => util.el('input[type="checkbox"]', brand).checked
        );
    ...
    class TagFilter extends Filter {
      ...
      html(tag) {
        return `<div class="md-radio"><input type="radio" id="${util.id(
          tag.name
        )}" data-name="${tag.name}" name="tag" value="${
          tag.parameter
        }" /><label for="${util.id(
          tag.name
        )}"><div class="-txt -radio-el" style="background-color:${
          tag.bg_color
        };color:${tag.text_color}">${tag.name}</div></label></div>`;
      }
    ...
    class DiscountFilter extends Filter {
      ...
      initialize() {
        ...
        this.TYPE = "radio";
        this.isDiscount = (evt) =>
          evt.target.getAttribute("type") === this.TYPE;
        this.header.removeEventListener("click", this.listen.bind(this));
      ...
      updateUi() {
        if (this.isUiInPlace === false) {
          this.discountEl.innerHTML = util.discounts
            .map(this.html.bind(this))
            .join("");
          pubsub.emit(util.FILTER_UPDATED, {});
          this.isUiInPlace = true;
      ...
      html(discount) {
        return `<div class="md-radio"><input type="radio" id="d${util.id(
          discount.name
        )}" name="discount" value="${discount.value}" /><label for="d${util.id(
          discount.name
        )}"><div class="-txt -radio-el">${discount.name}</div></label></div>`;
      }
    ...
    class RatingFilter extends Filter {
      ...
      updateUi() {
        if (this.isUiInPlace === false) {
          this.ratingEl.innerHTML = util.ratings
            .map(this.html.bind(this))
            .join("");
          pubsub.emit(util.FILTER_UPDATED, {});
          this.isUiInPlace = true;
    ...
    class PaginationFilter extends Filter {
      ...
      initialize() {
        ...
        this.isJmp2Page = (evt) => evt.target.classList.contains("-to-page");
      ...
      jumpToPage() {
        const value = this.pageNumbrEl.value;
  ...
  var util = new Util();
  var database = new Database(json);
