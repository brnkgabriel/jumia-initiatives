...
var Begin = (function (data) {
  class Util {
    constructor(json) {
      ...
      this.minute_duration = parseInt(json.config["minute_duration_" + this.SHEET_NAME])
      ...
      this.getData = (detail, json) => {
        const list = detail.json_list
        const key = json.key
        const name = json.name
        localStorage.setItem("list", JSON.stringify(list))
        return list.filter(datum => datum[key] === name)
      }
    ...
    platform() {
      ...
      const mobileBanner = this.config[this.SHEET_NAME + "_mobile_banner"]
      const desktopBanner = this.config[this.SHEET_NAME + "_desktop_banner"]
      const deeplink = this.config[this.SHEET_NAME + "_deeplink"]

      var banner = is_mobile ? mobileBanner : desktopBanner
      var live_link = is_mobile ? deeplink : (this.domain.host + '/' + this.config.download_apps_page)
  ...
  class Controller extends Util {
    constructor(json) {
      super(json)
      this.data = this.getData(json, { key: 'type', name: this.NAME })
...
var element_id = 'app'
var feature_box = Featurebox({ id: gsheet_id, config: fb_config, element_id, name: 'flash_sales' })
