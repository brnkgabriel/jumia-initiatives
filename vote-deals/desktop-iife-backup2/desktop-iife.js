var Begin = function (data) {
  ...
  class SKURows extends Util {
    ...
    //   Object.keys(this.fslsVotes).map(key => {
    //     var id = this.fslsVotes[key]
    //     if (id) {
    ...
    vote(evt) {
      ...
      if (target.classList.contains(this.VOTE_CLASS)) {
        ...
        var skuMap = this.fslsVotes[skuId]

        if (skuMap === undefined) {
          var json = {}
          json[this.user.email] = Date.now
          this.fslsVotes[skuId] = json
        } else {
          var emailMap = skuMap[this.user.email]

          if (emailMap === undefined || emailMap === this.UNVOTED) {
            this.fslsVotes[skuId][this.user.email] = Date.now()
          } else {
            this.fslsVotes[skuId][this.user.email] = this.UNVOTED
          }
        }

        this.updateVotes()
        feature_box.emit(this.SUBMIT, this.fslsVotes)
        // this.voteUpdates({sku, skuID, clicked: true})
        // var voteEls = this.voteEls(sku)
        // var json = this.voteClickUpdates({ ...voteEls, skuID })
        // this.voteUiUpdates({ ...voteEls })
        // this.jsonToLStorage(this.LS_KEY, this.fslsVotes)
      }
    }
    ...
    voteClickUpdates({ section, voteCountEl, target, skuID, sku, skusInSection }) {
      ...
      if(this.fslsVotes[groupID]) {
        delete this.fslsVotes[groupID]
        section.classList.remove("-voted")
        sku.classList.remove("-sku-voted")
        target.textContent = "vote"
        voteCount = voteCount - 1
    ...
    updateVotes() {
      Object.keys(this.fslsVotes).map(skuId => {
        var mapOfEmails = this.fslsVotes[skuId]
        ...
        this.updateImageAndCTA({ section, skuEl, skuCTA, mapOfEmails })
      })
    }
    ...
    updateImageAndCTA({ section, skuEl, skuCTA, mapOfEmails }) {
