var Featurebox = (function (json) {
  ...
  class Controller {
    ...
    updateDomain(json) {
      console.log("config is", json.config, "domain is", json.domain)
    ...
    initialize(json) {
      ...
      if (this.itemsToRender.indexOf(util.CATALOG) === -1) {
        ...
        console.log("observed without catalog")
      }
    }
    ...
    banners() {
      const bannerParent = util.el(".-banner.-row")
  ...
  const Filters = function () {
    ...
    class AppliedFilters extends Filter {
      ...
      updateUi() {
        const data = util.urlParams
        this.all.innerHTML = Object.keys(data)
        .filter(key => data[key] !== "")
        .map(key => Object.create({ key, value: data[key] }) )
